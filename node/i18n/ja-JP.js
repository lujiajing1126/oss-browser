module.exports = {
  ...
  "region.oss-cn-fuzhou": "福州地域 (中国東部 6)",
  ...
  "auth.presetOssPath.popup.msg1":
    "現在使用されている AK には、すべてのバケットのアクセス権があるので、[プリセット OSS パス] を設定する必要はありません。",
  "auth.presetOssPath.popup.msg2":
    "現在使用されている AK は、特定バケットまたはバケット配下の特定パスに対するアクセス権しかないので、[プリセット OSS パス] を設定する必要があります",

  "auth.remember.popup.msg1":
    "AK を保存するには、 [入力情報の保存] チェックボックスをチェックします。次にログインするときは、[AK 履歴] をクリックしてログインするキーを選択します。AK を手動で入力する必要はありません。 一時的に使用するコンピューターの場合、これをチェックしないでください",

  region: "リージョン",
  requestPay: "request payer",
  "requestpay.popup.msg":
    "あなたの許可されたバケットが要求支払人モードを開き、あなたがそのバケットの所有者ではない場合、あなたは「要求支払人モード」をチェックするべきです。 あなたがバケツを訪れたときにあなたが生み出すトラフィックやリクエストなどの量はあなたによって支払われるでしょう。詳細については、ヘルプドキュメントを参照してください",
  ...
  "auth.removeAK.message":
    "AK <code>{{id}}</code> を削除してもよろしいですか。",
  ...
  forbidden:
    "選択したオブジェクトを現在のフォルダーまたは Region 間で移動することはできません",
  ...
  "acl.warn-not-private.public-read":
    "<code>公開読み取り</code>権限は、匿名でバケット内のデータにアクセスでき、 セキュリティリスクが高くなるため、 非公開 (private) を推奨します。",
  ...
  "multipart.description.tooltip":
    "つまり、初期化されたが、完了または中止のイベントが来ていないマルチパートアップロードです",
  ...
  "bucket.delete.message":
    "バケット名: <code>{{name}}</code>, リージョン: <code>{{region}}</code>, このバケットを削除してもよろしいですか。",
  ...
  "settings.autoUpgrade.msg":
    "アップデートパッケージを自動的にダウンロードします",
  "settings.log": "ログ設定",
  ...
  "genAuthToken.message3.2":
    "このロールは、この{{type}}のアクセスには少なくとも{{privilege}}権限が必要です",

  "genAuthToken.message4": "認証トークン",
  "genAuthToken.message5":
    "上に生成された認証コードを使用して OSS ブラウザにログインすると、{{type}} [{{object}}] にアクセスする権限を得ることができます。{{expiration}} まで有効です。",
  ...
  "paste.message1":
    '<span class="text-info">{{name}}...</span> をこのディレクトリに<span class="text-info">{{action}}</span>しますか (同名のファイルまたはディレクトリはカバーされます)',

  "acl.update.title": "ACL の更新",
  "acl.update.success": "ACL を更新しました",
  "aclType.private.message":
    "非公開: オブジェクトに対するアクセスは認証が必要です",
  "aclType.public-read.message":
    "公開読み取り: オブジェクトに対する書き込みには認証が必要です。オブジェクトは誰でも読み取り可能です",
  "aclType.public-read-write.message":
    "公開読み書き: 誰でもオブジェクトを読み書き可能です",
  ...
  "codepreview.notsupport":
    "このファイルは直接オープンできません。ローカルにダウンロードしてからオープンしてください。",
  ...
  "ak.status.update.message.Active":
    "このアクセスキーを<code>無効化</code>してもよろしいですか。",
  "ak.status.update.message.Inactive":
    'このアクセスキーを<code class="text-success">有効化</code>してもよろしいですか。',
  "ak.delete.title": "AccessKey の削除",
  "ak.delete.message":
    "このアクセスキーを<code>削除</code>してもよろしいですか。",

  "user.update.message.tip":
    "AliyunRAMFullAccess 権限を持っていることを確認してください",
  "user.list.message.tip":
    "ここでは、必要なユーザー管理機能のみを提供しています。さらに拡張するには、RAM コンソールを操作してください:",
  ...
  "mail.test.message":
    'テストメッセージを <span class="text-primary">{{from}}</span> へ送信します',
